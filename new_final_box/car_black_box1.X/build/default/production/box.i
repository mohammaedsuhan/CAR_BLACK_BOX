...
void dashboard(void)
{
    ...
    {
        event_index1 = 8;
        store_event(event_index1);
    }
    else if (event_index1 == 8 && (key == 2 || key == 3))
    {
        event_index1 = 1;
        store_event(event_index1);
    }
    ...
    clcd_print(time, (0xC0 + (0)));
    clcd_print("  ", (0xC0 + (8)));
    clcd_print(event[event_index1], (0xC0 + (10)));
    clcd_print("  ", (0xC0 + (12)));
...
void store_event(int event_index2)
{
    ...
    for (int j = 0; j < 2; j++)
    {
        write_EEPROM(address++, event[event_index2][j]);
    }
    write_EEPROM(address++, speed % 10 + '0');
    write_EEPROM(address++, speed / 10 + '0');
    if (event_count % 10 == 0)
    {
        address = 0;
...
void display(void)
{
    ...
    clcd_print(opt[count], (0x80 + (2)));
    clcd_print(opt[count + 1], (0xC0 + (2)));
    if (flag == 0)
    {
        clcd_print("->", (0x80 + (0)));
        clcd_print("  ", (0xC0 + (0)));
...
void view_log(void)
{
    ...
    static unsigned int store_address = 0;
    static unsigned int temp_store = 0;
    if (temp_store < event_count && temp_store < 10)
    {
        unsigned int i = 0;
        while (i < 16)
        {
        ...
        stored_event[temp_store][i] = '\0';
    }
    temp_store++;
    if (event_index1 == 0)
    {
        clcd_print("NO LOGS         ", (0x80 + (0)));
        clcd_print("TO DISPLAY :    ", (0xC0 + (2)));
    }
    else
    {
        if (key == 11)
        ...
        clcd_print(stored_event[s_count], (0xC0 + (0)));
    }
    if (key == 2 && menu_flag == 2)
    {
        menu_flag = 1;
    }
}
...
void clear_log(void)
{
    ...
    clcd_print("Wait a minute    ", (0xC0 + (0)));
    event_index1 = 0;
    if (delay++ == 3000)
    {
        clcd_write(0x01, 0);
...
void update_time_in_ds1307(void)
{
    char hr_bcd = ((hour / 10) << 4) | (hour % 10);
    char min_bcd = ((min / 10) << 4) | (min % 10);
    char sec_bcd = ((sec / 10) << 4) | (sec % 10);
...
void download_log(void)
{
    static unsigned int download_flag = 0;
    if (download_flag < 10)
    {
        if (event_index1 == 0)
        {
        ...
        else
        {
            ...
            if (temp_store < event_count && temp_store < 10)
            {
                ...
                while (i < 14)
                {
                    putch(read_EEPROM(store_address));
                    store_address++;
                    if(i == 7 || i == 10)
                    {
                        putch(' ');
...
void set_time(void)
{
    static unsigned int SET_ONCE = 0;
    if(SET_ONCE == 0)
    {
        SET_ONCE = 2;
        clcd_write(0x01, 0);
    ...
    wait++;
    if (key == 11 && menu_flag == 5)
    {
        if (current_field == 0) hour = (hour + 1) % 24;
        else if (current_field == 1) min = (min + 1) % 60;
        else if (current_field == 2) sec = (sec + 1) % 60;
    }
    else if (key == 12 && menu_flag == 5)
    {
        current_field = (current_field + 1) % 3;
    }
    else if (key == 1 && menu_flag == 5)
    {
        update_time_in_ds1307();
        clcd_write(0x01, 0);
        SET_ONCE = 0;
        menu_flag = 1;
    }
    else if (key == 2 && menu_flag == 5)
    {
        clcd_write(0x01, 0);
        SET_ONCE = 0;
        menu_flag = 1;
